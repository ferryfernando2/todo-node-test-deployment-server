...
const MAX_REQUESTS_PER_MIN = Number(process.env.MAX_REQ_PER_MIN || 120);
const BAN_SECONDS = Number(process.env.BAN_SECONDS || 300);
// token-bucket parameters
const RATE_WINDOW_MS = 60 * 1000;
const BURST_MULTIPLIER = Number(process.env.RATE_BURST_MULTIPLIER || 3); // allow short bursts
...
try {
    ...
    const speed = slowDown({ windowMs: 60 * 1000, delayAfter: Math.round(MAX_REQUESTS_PER_MIN/2), delayMs: 500 });
    app.use(speed);
    app.use(limiter);
} catch (e) {}
...
if (multer) {
    ...
    app.post('/statuses', upload.single('file'), async (req, res) => {
        try {
            ...
            const statusObj = { id: `status_${Date.now()}`, userId, url, caption, created: new Date().toISOString(), expiresAt: new Date(Date.now() + 24*60*60*1000).toISOString() };
                ...
                switch (data.type) {
                    ...
                    case 'chat':
                        ...
                        try {
                            ...
                            if (recipientSockets.length > 0) {
                                // deliver to all active sockets for that user
                                for (const recipientWs of recipientSockets) {
                                    try {
                                        if (recipientWs && recipientWs.readyState === WebSocket.OPEN) {
                                            recipientWs.send(JSON.stringify({ type: 'chat', data: transientMsg }));
                                        }
                                ...
                                try { ws.send(JSON.stringify({ type: 'messageConfirmation', data: { id: transientMsg.id, status: 'delivered', tempId: transientMsg.tempId }, note: 'transient: delivered to recipient' })); } catch(e){}
                                try { ws.send(JSON.stringify({ type: 'messageStatus', data: { id: transientMsg.id, status: 'delivered' } })); } catch(e){}
                    ...
                    case 'group_chat':
                        try {
                            ...
                            // Persist the group message
                            try {
                                if (db && db.saveGroupMessage) {
                                    await db.saveGroupMessage(groupId, fromId, payload);
                                }
